...
class ThuPhiController {
	...
	createForm(req, res, next) {
		res.render("admin/KeToan/ThuPhi/them", {
			layout: "adminLayout",
			title: "Thêm khoản thu phí",
		});
	}
	...
	async store(req, res, next) {
		try {
			const { idCanHo, loaiPhi, soTien, trangThai, thang, nam, moTa } = req.body;
			const numericId = Number(idCanHo);

			// ————— Bước 1: Kiểm tra căn hộ tồn tại và không trống —————
			const canHoDoc = await CanHo.findOne({
				idCanHo: numericId,
				trangThai: { $ne: "trống" }, // hoặc trangThai: 'không trống' tùy schema
			}).lean();

			if (!canHoDoc) {
				// Nếu không hợp lệ, trả về form với lỗi
				return res.status(400).render("admin/KeToan/ThuPhi/them", {
					layout: "adminLayout",
					title: "Thêm khoản thu phí",
					error: `Căn hộ #${numericId} không tồn tại hoặc đang trống, không thể thu phí.`,
					// Đổ lại các giá trị người dùng vừa nhập:
					formData: { idCanHo, loaiPhi, soTien, trangThai, thang, nam, moTa },
				});
			}

			// ————— Bước 2: Tạo mới khoản phí —————
			await Phi.create({
				idCanHo: numericId,
				loaiPhi,
			...
			// Chuyển về danh sách thu phí
			res.redirect("/admin/thuphi");
		} catch (error) {
			next(error);
		}
	}
